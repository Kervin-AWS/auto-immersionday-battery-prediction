...
Remaining_cycles_scaling_factor = 2159.0

def my_acc_current_cycle(y_true, y_pred):
    y_pred = tf.convert_to_tensor(y_pred)
    y_true = tf.dtypes.cast(y_true, y_pred.dtype)
    diff = 1.0 - tf.math.abs(
        (y_true - y_pred) / K.clip(tf.math.abs(y_true), 1 / Remaining_cycles_scaling_factor, None))
    return K.mean(diff, axis=0)[0]


def my_acc_remaining_cycle(y_true, y_pred):
    y_pred = tf.convert_to_tensor(y_pred)
    y_true = tf.dtypes.cast(y_true, y_pred.dtype)
    diff = 1.0 - tf.math.abs(
        (y_true - y_pred) / K.clip(tf.math.abs(y_true), 1 / Remaining_cycles_scaling_factor, None))
    return K.mean(diff, axis=0)[1]
